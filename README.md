...
- **Relative value:** Compare `ret_12m` vs `vol_ann` to find stocks where momentum is "cheap" (high return, low vol)
- **Risk overlay:** Use `max_drawdown` and `from_52w_high` to gauge how far a stock is from its recent peak
- **Research baseline:** Use as a starting point for your own factor models — swap `ret_12m` for `ret_6m`, add size, add earnings yield, etc.

---

## 🔗 Full Pack (Extended)

This repo contains the **complete 151-stock dataset** for free. The [Whop Full Pack](https://whop.com/launchtower/launchtower-full-dataset-methodology/) ($29 one-time) adds:

- Dated research report with sector-level commentary
- Factor distribution charts
- Extended methodology notes (sensitivity to lookback window, z-score vs rank-based scoring)
- The same CSV + script, packaged with a changelog
...
Data: Public market data (Yahoo Finance). Code: MIT. Use freely, cite if you publish.
