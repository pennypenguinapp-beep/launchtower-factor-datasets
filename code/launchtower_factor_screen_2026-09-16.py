...
def main() -> None:
    ...
    if dropped:
        print(f"WARNING: dropped {len(dropped)} tickers: {dropped}")

    factors = compute_factors(closes)
    scores = build_scores(factors)

    print(f"\n{'='*80}")
    print(f"LaunchTower Factor Screen — {dt.date.today().isoformat()}")
    print(f"Universe: {len(scores)} tickers")
    print(f"{'='*80}\n")
    ...
    print(f"\n{'='*80}")
    print("BOTTOM 10 BY COMPOSITE SCORE:")
    print(scores.tail(TOP_N).to_string(index=False))

    out_csv = f"factors_{dt.date.today().isoformat()}.csv"
